...
class CD(D):
    def __init__(self, name, search_type, children):
        super(CD, self).__init__(name, search_type, children, cc=True)
...
def test_charge_conjugation():
    ...
    yield (try_decay,
           'a -> [b]CC [c]CC [d]CC',
           D(P('a'), '->', [CP('b'), CP('c'), CP('d')]))
    yield (try_decay,
           '[a]CC -> [b]CC [c]CC [d]CC',
           D(CP('a'), '->', [CP('b'), CP('c'), CP('d')]))
